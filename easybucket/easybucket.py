...
class Bucket:
    def __init__(self, bucket_name, btype="json", exit_callback=lambda:None):
...
class EasyBucket:
    def __init__(self, max_fifo):
        self.collection_lock = threading.Lock()
        self.buckets = {}
        self.fifo = []
        self.max_fifo = max_fifo

    def __call__(self, bucket_name, btype="json"):
        with self.collection_lock:
            bucket, cnt = self.buckets.get(bucket_name, (None, 0))
            if bucket==None:
                bucket = Bucket(bucket_name, btype=btype, exit_callback=self.untick)
